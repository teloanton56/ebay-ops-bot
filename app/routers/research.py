...
async def search(q: str = Query(min_length=2, max_length=120), marketplace_id: str | None = None,
    ...
    if marketplace_id and marketplace_id != "EBAY_US":
        raise HTTPException(400, "v0.23 recherche uniquement sur eBay US")
    try:
...
async def score_product(product_id: int, limit: int = Query(default=30, ge=1, le=200)):
    ...
    if product.get("marketplace_id") != "EBAY_US" or product.get("currency") != "USD":
        raise HTTPException(410, "Ce produit appartient à l'ancien catalogue. Re-sourcer en eBay US / USD.")
    _require_real_market()
    try:
...
def history(limit: int = 20):
    return [row for row in list_research(min(max(limit, 1), 100)) if row.get("marketplace") == "EBAY_US"]
