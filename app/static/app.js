(() => {
  ...
  function renderAutoDiscoveries(){const area=$('#autoTrendResults');if(!area)return;const latest=state.discoveries[0];if(!latest){area.className='empty-state compact';area.innerHTML='<strong>Aucun relevé automatique</strong><span>Connectez YouTube puis lancez la première détection.</span>';return}const themes=(latest.themes||[]).slice(0,12);area.className='trend-discovery';area.innerHTML=`<div class="trend-summary"><div><strong>${themes.length} thème(s) repéré(s) sur YouTube · ${esc(latest.country)}</strong><span>${new Date(latest.scanned_at).toLocaleString('fr-FR')} · fréquence publique, pas ventes</span></div><button class="mini-btn" data-go="connections">Sources</button></div><div class="trend-cloud">${themes.map(t=>`<button class="trend-chip" data-trend-keyword="${esc(t.keyword)}"><strong>${esc(t.keyword)}</strong><span>${esc(t.category)} · ${t.mentions} apparition(s)</span><span class="trend-score">${t.signal_score}/100 signal</span></button>`).join('')}</div><div class="discovery-guard">Un score élevé signifie seulement que le thème revient souvent dans l’échantillon observé. Confirmez ensuite la demande eBay, le coût livré, la conformité et le fournisseur.</div>`}
  async function runAutoDiscovery(){const button=$('#runAutoDiscovery'),area=$('#autoTrendResults');button.disabled=true;button.textContent='Détection en cours…';area.className='';area.innerHTML=loading('Lecture des vidéos populaires et extraction des thèmes…');try{const d=await fetchJson('/api/radar/discover',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({country:$('#autoTrendCountry').value})});state.discoveries=[d,...state.discoveries];renderAutoDiscoveries();toast(`${d.themes.length} thème(s) détecté(s)`);await loadOpportunities()}catch(e){area.innerHTML=`<div class="error-box">${esc(e.message)}</div>`}finally{button.disabled=false;button.textContent='✦ Détecter maintenant'}}
  function renderRadarWatch(){const area=$('#radarWatchlist');$('#navRadarCount').textContent=state.radarWatch.length;if(!state.radarWatch.length){area.className='empty-state compact';area.innerHTML='<strong>Aucun produit surveillé</strong><span>Ajoutez un mot-clé pour constituer votre liste de relevés.</span>';return}area.className='radar-watch-list';area.innerHTML=state.radarWatch.map(x=>`<div><span><strong>${esc(x.keyword)}</strong><small>${esc(x.notes||'Relevé automatique toutes les 6 h')}</small></span><button class="mini-btn primary" data-signal-watch="${esc(x.keyword)}">Réseaux</button><button class="mini-btn" data-scan-watch="${esc(x.keyword)}">eBay</button><button class="mini-btn" data-delete-watch="${x.id}">×</button></div>`).join('')}
  ...
  async function runRadarScan(){const query=$('#radarQuery').value.trim(),markets=$$('input[name="radar_market"]:checked').map(x=>x.value),area=$('#radarResults');if(!query||!markets.length){toast('Indiquez un produit et au moins un marché');return}area.className='';area.innerHTML=loading('Relevé des marketplaces…');try{const d=await fetchJson('/api/radar/scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({keyword:query,marketplaces:markets})});area.innerHTML=`<div class="info-box"><strong>Données mesurées uniquement.</strong><p>${esc(d.note)}</p></div><div class="radar-market-results">${d.markets.map(m=>`<article><div class="radar-market-head"><strong>${esc(m.marketplace_name)}</strong><span>${m.history_available?(m.listing_change_percent===null?'Historique disponible':`${m.listing_change_percent>=0?'+':''}${m.listing_change_percent} % d’annonces`):'Premier relevé'}</span></div><div class="radar-market-metrics"><div><small>Annonces</small><strong>${m.total_results}</strong></div><div><small>Prix médian</small><strong>${money(m.median_price,m.currency||'EUR')}</strong></div><div><small>Vendeurs observés</small><strong>${m.sellers_sample}</strong></div><div><small>Top vendeur</small><strong>${esc(m.top_seller||'—')}</strong></div></div><p>Recherche exacte : non publique · Conversion concurrente : non publique</p></article>`).join('')}</div>`}catch(e){area.innerHTML=`<div class="error-box">${esc(e.message)}</div>`}}
  ...
  document.addEventListener('click', e=>{
    ...
    const scanWatch=e.target.closest('[data-scan-watch]');if(scanWatch){$('#radarQuery').value=scanWatch.dataset.scanWatch;runRadarScan();return;}
    ...
    const trend=e.target.closest('[data-trend-keyword]');if(trend){$('#radarSignalQuery').value=trend.dataset.trendKeyword;$('#radarSignalForm').scrollIntoView({behavior:'smooth',block:'center'});toast('Thème prêt à être recoupé');return;}
