...
def ebay_series(orders: list[dict], products: list[dict], days: int,
    ...
    product_by_sku = {str(item.get("supplier_sku") or ""): item for item in products}
    ...
    for order in orders:
        ...
        if not row:
            continue
        revenue = float(((order.get("pricingSummary") or {}).get("total") or {}).get("value") or 0)
        row["revenue"] += revenue
        row["orders"] += 1
        unknown_revenue = 0.0
        ...
        if unknown_revenue:
            row["supplier_cost"] += unknown_revenue * 0.31
            row["shipping_cost"] += unknown_revenue * 0.14
        ...
        row["fixed_fees"] += settings.default_fixed_fee
    for row in rows.values():
        costs = sum(row[key] for key in ("supplier_cost", "shipping_cost", "ebay_fees", "ad_fees",
                                         "returns_reserve", "fixed_fees"))
        row["net_result"] = row["revenue"] - costs
    completeness = round(known_lines / total_lines * 100, 1) if total_lines else 100.0
    return [_round_day(row) for row in rows.values()], {"known_lines": known_lines,
...
def summarize(series: list[dict], *, days: int, target: int,
    ...
    milestones = [{"amount": amount, "reached": revenue >= amount,
                   "progress_percent": round(min(revenue / amount * 100, 100), 1),
                   "remaining": round(max(amount - revenue, 0), 2)} for amount in MILESTONES]
    costs = {"supplier": totals["supplier_cost"], "shipping": totals["shipping_cost"],
             "ebay": totals["ebay_fees"], "ads": totals["ad_fees"],
             "returns": totals["returns_reserve"], "fixed": totals["fixed_fees"]}
    return {
        "source": source,
        "currency": "EUR",
        "period_days": days,
        ...
        "disclaimer": "Résultat estimé pour le pilotage, hors TVA, impôts, remboursements réels et comptabilité officielle.",
    }
