...
from app.services.db import (add_alert, finish_analysis_run, get_product, list_products,
                             save_analysis_result, set_product_fields, start_analysis_run)
        ...
        for product in products:
            try:
                ...
                if live:
                    ...
                    # Without real marketplace data, only deterministic catalogue/risk checks run.
                    # A product is never promoted to Winner from invented demand signals.
                ...
                profit = risk["profit"]
                save_analysis_result(run_id, product["id"], score=score, suggested_price=price,
                                     margin_percent=profit["margin_percent"], estimated_profit=profit["estimated_profit"],
                                     previous_status=product.get("product_status"), new_status=new_status)
                if new_status == "Winner":
                    winners += 1
                    if live:
                        add_alert(product["id"], "success", "WINNER", f"Nouveau Winner détecté sur données eBay : score {score:.0f}/100.")
                elif new_status == "Rejeté":
                    rejected += 1
                    add_alert(product["id"], "danger", "RISK", "Produit rejeté automatiquement : " + (" ; ".join(risk["blocks"]) or f"score {score:.0f}/100"))
                analyzed += 1
            except Exception as exc:
                errors += 1
                save_analysis_result(run_id, product["id"], previous_status=product.get("product_status"), error=str(exc))
                add_alert(product["id"], "danger", "ANALYSIS_ERROR", f"Analyse impossible : {exc}")
        finish_analysis_run(run_id, analyzed, winners, rejected, errors)
        return {"run_id": run_id, "mode": "EBAY" if live else "CATALOGUE", "products_total": len(products),
                "products_analyzed": analyzed, "winners": winners, "rejected": rejected, "errors": errors,
                "market_data": live, "dry_run": True}
