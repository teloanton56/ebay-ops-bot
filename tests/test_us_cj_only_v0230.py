...
def product_payload(**overrides):
    data = {
        "supplier_sku": "CJ-US-TEST",
        "title": "Car seat organizer",
        "description": "Generic car organizer sourced through CJ Dropshipping.",
        "supplier_cost": 6.0,
        "shipping_cost": 4.0,
        "stock": 25,
        "shipping_days": 5,
...
def test_cj_route_policy_is_us_first_and_cn_is_stricter():
    ...
    assert cn == {
        ...
        "min_profit": 8.0,
        "min_stock": 20,
        "max_shipping_days": 12,
    }

    routes = {
        "US": {"warehouse": "US", "supplier_cost": 6, "shipping_cost": 4, "stock": 25, "shipping_days": 5},
        "CN": {"warehouse": "CN", "supplier_cost": 4, "shipping_cost": 5, "stock": 40, "shipping_days": 10},
    }
    selected = select_cj_route(routes, reference_price=30)
    assert selected["warehouse"] == "US"
    assert selected["eligible"] is True


def test_china_route_is_only_eligible_with_stricter_economics():
    good = evaluate_route(
        {"warehouse": "CN", "supplier_cost": 4, "shipping_cost": 5, "stock": 40, "shipping_days": 10},
        30,
    )
    bad_profit = evaluate_route(
        {"warehouse": "CN", "supplier_cost": 8, "shipping_cost": 6, "stock": 40, "shipping_days": 10},
        20,
    )
    bad_delay = evaluate_route(
        {"warehouse": "CN", "supplier_cost": 4, "shipping_cost": 5, "stock": 40, "shipping_days": 14},
        30,
    )
    assert good["eligible"] is True
    assert good["profit"]["estimated_profit"] >= 8
    assert good["profit"]["margin_percent"] >= 30
    assert bad_profit["eligible"] is False
    assert bad_delay["eligible"] is False


def test_risk_engine_uses_route_specific_thresholds():
    cn_product = product_payload(
        supplier_sku="CJ-CN-TEST",
        supplier_cost=4,
        shipping_cost=5,
        stock=30,
        shipping_days=10,
    ...
    assert risk["pass"] is True
    assert risk["route"]["warehouse"] == "CN"
    assert risk["route"]["requirements"]["max_shipping_days"] == 12

    low_profit = assess_product({**cn_product, "target_price": 20})
    assert low_profit["pass"] is False
    assert any("Profit estimé" in block for block in low_profit["blocks"])
    ...
    assert us_risk["pass"] is False
    assert any("Délai 8j > maximum 7j" in block for block in us_risk["blocks"])


def test_products_api_hides_legacy_fr_catalogue_and_forces_usd():
    legacy_id = db.upsert_product(product_payload(
        supplier_sku="OLD-FR-1",
    ...
    assert rows[0]["marketplace_id"] == "EBAY_US"
    assert rows[0]["currency"] == "USD"


def test_retired_sources_are_rejected_by_active_api():
    with TestClient(app) as client:
        ali = client.post("/api/supplier-flow/add", json={
            "provider": "aliexpress",
            "supplier_sku": "ALI-1",
            "name": "Example",
            "price": 5,
        })
        social = client.post("/api/connections/signals/scan", json={
            "keyword": "organizer",
    ...
    assert ali.status_code == 410
    assert social.status_code == 410
    assert discover.status_code == 410
    assert connections.status_code == 200
    assert connections.json()["operating_mode"] == "EBAY_US_CJ_ONLY"
    assert connections.json()["sources"] == []


def test_settings_api_ignores_old_marketplace_and_currency():
    with TestClient(app) as client:
        response = client.post("/api/settings/ebay", json={
            "environment": "sandbox",
            "marketplace_id": "EBAY_FR",
            "currency": "EUR",
        })
    ...
    assert response.json()["marketplace_id"] == "EBAY_US"
    assert response.json()["currency"] == "USD"
    assert current.json()["marketplace_id"] == "EBAY_US"
    assert current.json()["currency"] == "USD"
...
def test_active_sourcing_frontend_names_only_cj():
    supplier_flow = read("app/static/supplier_flow_v2.js").lower()
    margin_hunter = read("app/static/margin_hunter.js").lower()
    shop_spy = read("app/static/shop_spy.js").lower()
    cleanup = read("app/static/provider_cleanup.js").lower()

    assert "cj dropshipping" in supplier_flow
    assert "aliexpress" not in supplier_flow
    assert "amazon" not in supplier_flow
    assert "cj only" in margin_hunter
    assert "aliexpress" not in margin_hunter
    assert "amazon" not in margin_hunter
    assert "comparer avec cj" in shop_spy
    assert "aliexpress" not in shop_spy
    assert all(name in cleanup for name in ("amazon", "aliexpress", "tiktok", "youtube"))
...
def test_listing_and_supplier_refresh_require_route_specific_locations():
    ...
    assert 'marketplaceId": "EBAY_US"' in listing
    assert '"currency": "USD"' in listing
