...
def test_cj_us_candidate_uses_real_us_freight_and_hits_margin_goal():
    class FakeCJ:
        async def product_detail(self, pid):
            ...
            return {
                ...
                "variants": [{
                    ...
                    "inventories": [{"country_code": "US", "stock": 100}],
                }],
            }
        ...
        async def freight_options(self, vid, *, start_country, destination_country, storage_ids=None):
            ...
            return [
                {"name": "US Fast", "price_usd": 2.0, "delivery_days": "3-5 Days"},
...
def test_cj_china_candidate_is_allowed_only_under_stricter_thresholds():
        ...
        async def product_detail(self, pid):
            return {
                ...
                "variants": [{
                    ...
                    "price_usd": 3.0,
                    "stock": 50,
                    "inventories": [{"country_code": "CN", "stock": 50}],
                }],
            }
    ...
    good, error = asyncio.run(margin_hunter._deep_cj_candidate(
        FakeCJ(), product, reference_price=30.0, market=market, semaphore=asyncio.Semaphore(1)
    ))
    assert error is None
    assert good["warehouse"] == "CN"
    assert good["route_eligible"] is True
    assert good["verdict"] == "CHINE RENTABLE"

    weak, error = asyncio.run(margin_hunter._deep_cj_candidate(
        FakeCJ(), product, reference_price=14.0, market=market, semaphore=asyncio.Semaphore(1)
    ))
    assert error is None
    assert weak["warehouse"] == "CN"
    assert weak["route_eligible"] is False
    assert weak["goal_hit"] is False
    assert weak["score"] <= 54
    assert weak["verdict"] == "REJETER"
...
def test_margin_hunter_is_ebay_us_cj_only_in_router_and_ui():
    ...
    assert "aliexpress" not in service.lower()
    assert "amazon" not in service.lower()
    assert "CJ ONLY" in ui
    assert "eBay US" in ui
    assert "aliexpress" not in ui.lower()
    assert "amazon" not in ui.lower()
